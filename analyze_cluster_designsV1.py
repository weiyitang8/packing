...
# extract the sequences, (singles sequence can have multiple molecular models), and cluster by BLOSOM matrix
...
score_features = {'dG_separated':[] , 'delta_unsatHbonds':[], 'hbonds_int':[], 'holes-ABX_interface':[] , 'sc_value':[]}
...
score_Axis 		= {'dG_separated':'Association Energy (REU)' , 'delta_unsatHbonds':"Unsaturated Buried Polar Groups", 'hbonds_int':"# Interface H-bonds", 'holes-ABX_interface':"RosettaHoles2" , 'sc_value':"Interface Shape Complementarity"}
...
for k,v in score_features.items():
	...
	elif k == 'holes-ABX_interface':	# limit for RosettaHoles2 score is 1.5 stdev lower than mean score or < 0.1
...
score_keys = [ 'holes-ABX_interface', 'dG_separated', 'sc_value', 'hbonds_int', 'delta_unsatHbonds' ]
...
for d in all_designs:
	...
	if d.passed_filters >3:
		...
		print (d, d.passed_filters, sorted( d.score_feat.items() ) )
		...
		#newname
		d.new_design_label = 'design_%s_%s.pdb' % ( os.path.dirname(sys.argv[1]).split('_')[0].split('/')[1] , d )
		print('################',os.path.dirname(sys.argv[1]))
		print('################',os.path.dirname(sys.argv[1]).split('_'))

		d.score_summary = score_summary
		score_summary.append( d.new_design_label )
		print('#############################',d.new_design_label)
		...
		if not os.path.exists(outfile):
			print(d.filePath)
			print(outfile)
			shutil.copyfile( d.filePath, outfile)
