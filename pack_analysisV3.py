...
class Hole:
	...
	def findknobs(self,chainX,contactparm=3):
...
def excute(protein,chainABC_str,chainX_str,x,y=0): #chainABC_str has to be 2 letters with NO spacing in between, chainX_str has to be one letter, x is the cut-off for the final score.
...
#########################################################
	...
	for chain in xholes:
		for hole in chain:
			if hole.eli== 1:
				hole.findknobs(chainA)
				hole.findknobs(chainB)
				if hole.knobsc>0:
					# print(hole.id,hole.eli,prepackproduct, hole.knobsc,['knob'+str(knob.get_full_id()[2])+str(knob.get_id()[1]) for chain in hole.knob for knob in chain])
   ...
   #filter to process the protein with SASA method
		...
		for nholes in holes:
			for chain in nholes:
				for hole in chain:
					if hole.knobsc>0:
						# print('x')
						result.append([hole.id,getknobname(hole.knob)])
	...
	return sum1,sum2,sum3,sum4,sum5,sum6,finalscore,result
